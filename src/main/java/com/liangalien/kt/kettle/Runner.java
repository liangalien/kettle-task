...
public class Runner {
    ...
    public void execute(TaskDTO taskDto, RunnerTrigger trigger, String createBy) {
        ...
        try {
            ...
            LogListener.writeLog(name + " - 已就绪", LogLevel.BASIC, 0L);
            ...
            if (RunnerType.TRANS.getValue().equals(taskDto.getRepoType())) {
                ...
                if (lastTrans != null && !lastTrans.isFinishedOrStopped()) {
                    LogListener.writeLog(name + " - 上次执行未结束，无法执行", LogLevel.ERROR, 0L);
                    return;
                }
                ...
                LogListener.writeLog(name + " - 准备执行", LogLevel.BASIC, 0L);
                trans.prepareExecution(null);

                LogListener.writeLog(name + " - 开始执行", LogLevel.BASIC, 0L);
            ...
            LogListener.writeLog(name + " - 执行异常：" + e.getMessage(), LogLevel.ERROR, 0L);
            ...
            LogListener.writeLog(name + " - 执行结束，状态为：" + endStatus.getDesc(), LogLevel.BASIC, 0L);
            if (runnerName != null) {
    ...
    public static boolean transStop(String taskKey) {
        ...
        if (!trans.isFinishedOrStopped()) {
            ...
            LogListener.writeLog("用户强制停止当前转换！", LogLevel.ERROR, 0L);
    ...
    public static boolean jobStop(String taskKey) {
        ...
        if (!job.isFinished() && !job.isStopped()) {
            ...
            LogListener.writeLog("用户强制停止当前作业！", LogLevel.ERROR, 0L);
